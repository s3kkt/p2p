...
func ParseFlags() (string, string) {
	...
	gaiadPath := flag.String("gaiad-bin-path", "/usr/sbin/gaiad", "Path to expose metrics.")
	flag.Parse()
	return *listenPort, *metricsPath
}

func GetGaiaStatus() string {
	...
	service := "/usr/sbin/gaiad"
	argument := "status"
	command := exec.Command(service, argument)
	return JSONPayload
}
...
func handleMetrics() {
	...
	for {
		status := GetGaiaStatus()
		json.Unmarshal([]byte(status), &gaiaStatus)
...
func main() {
	go handleMetrics()
	metricsServe(":"+listenPort, metricsPath)
}
